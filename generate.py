...
def findPatchHorizontal(refBlock, texture, blocksize, overlap, tolerance):
	...
	for i, j in product(range(H-blocksize), range(W-blocksize)):
		# calculate rms of the difference in the overlap area
		# (right side of the refblock and left side of the kernel in the texture)
...
def getMinCutPatchHorizontal(block1, block2, blocksize, overlap):
	...
	'''
	err = ((block1[:, -overlap:] - block2[:, :overlap])**2).mean(2)  # the overlap area diff rms
...
def getMinCutPatchBoth(refBlockLeft, refBlockTop, patchBlock, blocksize, overlap):
	...
	mask2[:overlap, :overlap] = np.maximum(mask2[:overlap, :overlap] - mask1[:overlap, :overlap], 0)  # removing from mask2 the overlap parts of mask1 that are positive
...
def generateTextureMap(image, blocksize, overlap, outH, outW, tolerance):
	nH = int(ceil((outH - blocksize) * 1.0 / (blocksize - overlap)))  # number of blocks in output image
